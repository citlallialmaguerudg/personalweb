...
Salidas (en assets/):
  CV_Citlalli_Almaguer_Gomez_es.pdf            CV completo, español
  CV_Citlalli_Almaguer_Gomez_en.pdf            Full CV, English
  CV_Citlalli_Almaguer_Gomez_es_sintetico.pdf  CV sintético (~2 págs), español
  CV_Citlalli_Almaguer_Gomez_en_short.pdf      Short CV (~2 pp), English

Uso:  python tools/build_cv.py        (requiere: pip install reportlab pillow)
"""
import json, re, sys
...
T = {  # rótulos de sección
 ...
 'review_h':  {'es':'Dictaminación y arbitraje','en':'Review & assessment'},
...
def circular_photo():
    """Recorta assets/citlalli.png en círculo (PNG con alfa) para el encabezado."""
    src = ROOT / 'assets' / 'citlalli.png'
...
def build_full(D, lang, path):
    ...
    for p in [x for x in D['projects'] if x.get('active')]:
        me = next((c for c in p.get('collaborators', []) if 'Almaguer' in c.get('name','')), None)
...
def build_short(D, lang, path):
    ...
    if D.get('conferences'):
        s += sec(T['conferences'][lang])
        intl = [c for c in D['conferences'] if any(k in c['name'] for k in ('Microoptics','Applications of Optics','Information Optics','International Commission','European Meeting','Education and Training','ICO'))][:5]
        for c in intl:
...
if __name__ == '__main__':
    ...
    for lang in ('es', 'en'):
        ...
        globals()['FOOT'] = FOOT
        full = out / f"CV_Citlalli_Almaguer_Gomez_{lang}.pdf"
        short = out / (f"CV_Citlalli_Almaguer_Gomez_es_sintetico.pdf" if lang == 'es'
                       else "CV_Citlalli_Almaguer_Gomez_en_short.pdf")
        build_full(D, lang, full)
        build_short(D, lang, short)
        print('OK', full.name, '·', short.name)
